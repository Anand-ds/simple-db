...
Append‑only durability — every write is preserved
Human‑readable log — the database is a text file
Supports arbitrary values — JSON, XML, blobs, anything
Zero dependencies — works anywhere Bash does
...
Event sourcing
LSM‑tree storage engines (used by LevelDB, RocksDB, Cassandra)

The difference?
Here, the entire “database engine” fits in a few lines of Bash.

git clone https://github.com/anand_ds/simple-db.git
cd simple-db
chmod +x db.sh

# Make sure you are in the 'simple-db' directory before sourcing,
